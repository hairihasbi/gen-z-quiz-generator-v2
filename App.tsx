...
const ApiMonitorModal = ({ onClose }: { onClose: () => void }) => {
    ...
    useEffect(() => { setStats(getApiKeyStats()); const interval = setInterval(() => { setStats(getApiKeyStats()); }, 2000); return () => clearInterval(interval); }, []);
    const totalUsage = stats.reduce((acc, s) => acc + s.usageCount, 0);
    const totalErrors = stats.reduce((acc, s) => acc + s.errorCount, 0);
    return (<div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in-up"><div className="bg-white dark:bg-slate-800 w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"><div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center"><div><h3 className="text-xl font-bold text-slate-800 dark:text-white flex items-center gap-2"><Cpu className="text-brand-500" /> API Key Health Monitor</h3><p className="text-xs text-slate-500">Real-time usage & rotation status</p></div><button onClick={onClose} className="p-2 bg-slate-100 dark:bg-slate-700 rounded-full hover:bg-slate-200 dark:hover:bg-slate-600"><X size={20} className="text-slate-500 dark:text-slate-400"/></button></div><div className="p-6 bg-slate-50 dark:bg-slate-900 grid grid-cols-3 gap-4 border-b border-slate-100 dark:border-slate-700"><div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm text-center"><p className="text-xs text-slate-400 uppercase font-bold">Active Pool</p><p className="text-2xl font-bold text-brand-600">{stats.length}</p></div><div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm text-center"><p className="text-xs text-slate-400 uppercase font-bold">Total Requests</p><p className="text-2xl font-bold text-green-600">{totalUsage}</p></div><div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm text-center"><p className="text-xs text-slate-400 uppercase font-bold">Errors / Rate Limits</p><p className="text-2xl font-bold text-red-500">{totalErrors}</p></div></div><div className="flex-1 overflow-y-auto p-6 space-y-4">{stats.map((s, idx) => (<div key={idx} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center justify-between"><div className="flex items-center gap-4"><div className={`w-3 h-3 rounded-full ${s.status === 'ACTIVE' ? 'bg-green-500 animate-pulse' : s.status === 'RATE_LIMITED' ? 'bg-orange-500' : 'bg-red-500'}`} /><div><p className="font-mono text-sm font-bold text-slate-700 dark:text-slate-300">{s.keyMask}</p><p className="text-[10px] text-slate-400">Last used: {s.lastUsed ? new Date(s.lastUsed).toLocaleTimeString() : 'Never'}</p></div></div><div className="flex gap-6 text-right"><div><p className="text-[10px] text-slate-400 uppercase font-bold">Usage</p><p className="font-bold text-slate-800 dark:text-white">{s.usageCount}</p></div><div><p className="text-[10px] text-slate-400 uppercase font-bold">Status</p><span className={`text-xs font-bold px-2 py-0.5 rounded ${s.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : s.status === 'RATE_LIMITED' ? 'bg-orange-100 text-orange-700' : 'bg-red-100 text-red-700'}`}>{s.status}</span></div></div></div>))}{stats.length === 0 && (<div className="text-center py-10 text-slate-400">No keys found. Check API_KEY environment variable.</div>)}</div></div></div>);
};

interface PricingPackage { name: string; price: number; credits: number; color: string; }
const DonationModal = ({ onClose, user, selectedPackage }: { onClose: () => void, user: User, selectedPackage?: PricingPackage | null }) => {
    ...
    const accountInfo = { DANA: { number: '0852-4848-1527', name: 'Gen-Z Admin', color: 'bg-[#118EE9]', text: 'text-[#118EE9]' }, SHOPEE: { number: '0852-4848-1527', name: 'Gen-Z Admin', color: 'bg-[#EE4D2D]', text: 'text-[#EE4D2D]' } };
    const handleCopy = (text: string) => { navigator.clipboard.writeText(text); alert("Nomor tersalin!"); };
    const handleConfirm = () => { let msg = ''; if (selectedPackage) { msg = `Halo Admin Gen-Z Quiz, saya *${user.username}* ingin membeli *${selectedPackage.name}* (${selectedPackage.credits} Credits) seharga Rp ${selectedPackage.price.toLocaleString('id-ID')} via ${method}. Mohon diproses. Terima kasih!`; } else { msg = `Halo Admin Gen-Z Quiz, saya *${user.username}* telah mengirim donasi via ${method}. Mohon cek dan tambah credit saya. Terima kasih!`; } window.open(`https://wa.me/6285248481527?text=${encodeURIComponent(msg)}`, '_blank'); };
    return (<div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in-up"><div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl overflow-hidden relative"><button onClick={onClose} className="absolute top-4 right-4 p-2 bg-slate-100 dark:bg-slate-700 rounded-full text-slate-500 hover:text-slate-800 transition-colors"><X size={20} /></button><div className="p-6 pb-0 text-center"><div className="w-16 h-16 bg-pink-100 text-pink-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-bounce">{selectedPackage ? <Gem size={32} fill="currentColor"/> : <Heart size={32} fill="currentColor" />}</div><h3 className="text-xl font-bold text-slate-800 dark:text-white">{selectedPackage ? 'Pembelian Paket' : 'Dukungan & Top Up'}</h3>{selectedPackage ? (<div className="mt-3 p-3 bg-brand-50 border border-brand-100 rounded-xl"><p className="text-sm font-bold text-brand-800">{selectedPackage.name}</p><p className="text-2xl font-bold text-brand-600 mt-1">Rp {selectedPackage.price.toLocaleString('id-ID')}</p><p className="text-xs text-brand-600 mt-1">+{selectedPackage.credits} Credits</p></div>) : (<p className="text-sm text-slate-500 mt-2">Dukungan Anda membantu server kami tetap hidup. Credit akan ditambahkan setelah konfirmasi.</p>)}</div><div className="p-6"><div className="flex p-1 bg-slate-100 dark:bg-slate-700 rounded-xl mb-6"><button onClick={() => setMethod('DANA')} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${method === 'DANA' ? 'bg-white dark:bg-slate-600 shadow-sm text-[#118EE9]' : 'text-slate-500'}`}><Wallet size={16} /> DANA</button><button onClick={() => setMethod('SHOPEE')} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${method === 'SHOPEE' ? 'bg-white dark:bg-slate-600 shadow-sm text-[#EE4D2D]' : 'text-slate-500'}`}><Wallet size={16} /> ShopeePay</button></div><div className={`rounded-2xl p-6 border-2 transition-colors ${method === 'DANA' ? 'border-[#118EE9]/20 bg-[#118EE9]/5' : 'border-[#EE4D2D]/20 bg-[#EE4D2D]/5'}`}><div className="flex flex-col items-center"><div className="text-center w-full"><p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Transfer Manual ke</p><div className="flex items-center justify-center gap-2 mb-1"><span className={`text-xl font-bold font-mono ${accountInfo[method].text}`}>{accountInfo[method].number}</span><button onClick={() => handleCopy(accountInfo[method].number)} className="p-1.5 hover:bg-black/5 rounded-md transition-colors"><Copy size={14} className="text-slate-400"/></button></div><p className="text-sm font-medium text-slate-600 dark:text-slate-300">a.n {accountInfo[method].name}</p></div></div></div><button onClick={handleConfirm} className="w-full mt-6 bg-[#25D366] hover:bg-[#20bd5a] text-white py-3.5 rounded-xl font-bold shadow-lg shadow-green-500/20 flex items-center justify-center gap-2 transition-all active:scale-95"><MessageCircle size={20} /> Konfirmasi Pembelian WA</button></div></div></div>);
};
...
const Dashboard = ({ user }: { user: User }) => {
  ...
  const packages: PricingPackage[] = [{ name: 'Paket Starter', price: 50000, credits: 50, color: 'bg-blue-50 border-blue-100 text-blue-700' }, { name: 'Paket Pro', price: 100000, credits: 100, color: 'bg-gradient-to-br from-brand-50 to-orange-50 border-brand-200 text-brand-700' }];
  useEffect(() => { if (!localStorage.getItem('genz_tour_completed')) { const t = setTimeout(() => setRunTour(true), 1500); return () => clearTimeout(t); } }, []);
  useEffect(() => { const loadStats = async () => { const userId = user.role === Role.ADMIN ? undefined : user.id; const quizzes = await dbService.getQuizzes(userId); const totalQuestions = quizzes.reduce((acc, q) => acc + (q.questions ? q.questions.length : 0), 0); setStats({ quizCount: quizzes.length, generated: totalQuestions }); const last7Days = Array.from({ length: 7 }, (_, i) => { const d = new Date(); d.setDate(d.getDate() - (6 - i)); return d; }); const processedData = last7Days.map(date => { const dayName = date.toLocaleDateString('id-ID', { weekday: 'short' }); const dailyQuizzes = quizzes.filter(q => { const qDate = new Date(q.createdAt); return qDate.getDate() === date.getDate() && qDate.getMonth() === date.getMonth() && qDate.getFullYear() === date.getFullYear(); }); return { name: dayName, generated: dailyQuizzes.length, published: dailyQuizzes.filter(q => q.status === 'PUBLISHED').length }; }); setChartData(processedData); const s = await dbService.getSettings(); setSettings(s); }; loadStats(); }, [user]);
...
const QuizResultView = ({ quiz, onClose }: { quiz: Quiz, onClose: () => void }) => {
  ...
  const downloadWord = () => {
     ...
     <style>body{font-family:'Times New Roman', serif} .question{margin-bottom:1em} .options{margin-left:1em} .type-header{font-weight:bold; text-transform:uppercase; margin-top:1.5em; margin-bottom:1em; text-decoration:underline;}</style>
     </head><body>
        ...
        body += `</tbody></table>`;
         ...
         quiz.questions.forEach((q, i) => {
             ...
             if (q.type !== currentType) {
                 currentType = q.type;
                 body += `<div class='type-header'>${getQuestionTypeLabel(q.type)}</div>`;
             }
             ...
             // RENDER OPTIONS FOR BOTH MULTIPLE CHOICE, COMPLEX MC, AND TRUE_FALSE
             if(q.type === QuestionType.MULTIPLE_CHOICE || q.type === QuestionType.COMPLEX_MULTIPLE_CHOICE || q.type === QuestionType.TRUE_FALSE) {
                 ...
                 // For TRUE_FALSE, if options are empty, default to ['Benar', 'Salah']
                 const opts = (q.type === QuestionType.TRUE_FALSE && (!q.options || q.options.length === 0)) ? ['Benar', 'Salah'] : (q.options || []);

                 if (opts.length > 0) {
                     body += `<div class='options'>`;
                     opts.forEach((opt, idx) => {
                         let bullet = `${String.fromCharCode(65+idx)}.`;
                         if (q.type === QuestionType.COMPLEX_MULTIPLE_CHOICE) bullet = '&#9744;'; // Checkbox
                         if (q.type === QuestionType.TRUE_FALSE) bullet = '&#9711;'; // Circle
    ...
    <div id="quiz-result-view" className="bg-graph-paper rounded-none h-full flex flex-col fixed inset-0 z-50 overflow-hidden">
       ...
       <div className="bg-white border-b border-slate-200 px-6 py-3 flex justify-between items-center shadow-sm no-print z-50">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button onClick={() => handlePrint(false)} className="px-3 py-1.5 text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors">
                <Printer size={14}/> Cetak Soal
            </button>
            <button onClick={() => handlePrint(true)} className="px-3 py-1.5 text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-lg text-xs font-bold flex items-center gap-1.5 transition-colors">
                <BookOpenCheck size={14}/> Cetak + Kunci
            </button>
                 ...
                 <>
                    ...
                    <div className="space-y-6 font-serif text-black">
                        {quiz.questions.map((q, idx) => {
                            ...
                            // LOGIC FOR TYPE HEADER:
                            // Show header if it's the first question OR if the type is different from the previous one
                                ...
                                <div key={q.id || idx} className="avoid-break group">
                                    ...
                                    {showTypeHeader && (
                                        <div className="mt-6 mb-4">
                                            <h3 className="font-bold text-sm uppercase underline decoration-2 underline-offset-4 tracking-wider">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            {/* Options */}
                                            {(q.type === QuestionType.MULTIPLE_CHOICE || q.type === QuestionType.COMPLEX_MULTIPLE_CHOICE || q.type === QuestionType.TRUE_FALSE) && (
                                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1 ml-2 print-grid">
                                                    {(q.type === QuestionType.TRUE_FALSE && (!q.options || q.options.length === 0) ? ['Benar', 'Salah'] : (q.options || [])).map((opt, i) => (
                                                        <div key={i} className="flex gap-2 items-baseline">
                                                            <span className="font-bold uppercase text-sm w-4">
                                                                {q.type === QuestionType.COMPLEX_MULTIPLE_CHOICE ? '☐' : (q.type === QuestionType.TRUE_FALSE ? '◯' : String.fromCharCode(65 + i) + '.')}
                                                            </span>
                                    ...
                                    {/* Key (Only Visible if Toggled) - Removed no-print class so it prints when toggled on */}
                                    {showKey && (
                                        <div className="mt-2 ml-10 p-2 bg-green-50 text-xs text-green-800 border-l-2 border-green-500">
...
const CreateQuiz = ({ user, onUpdateCredits }: { user: User; onUpdateCredits: (credits: number) => void }) => { const navigate = useNavigate(); const [loading, setLoading] = useState(false); const [progress, setProgress] = useState(0); const [loadingStep, setLoadingStep] = useState(''); const [generatedQuiz, setGeneratedQuiz] = useState<Quiz | null>(null); const [subjectCategory, setSubjectCategory] = useState("Wajib Umum"); const [subject, setSubject] = useState(SUBJECTS["Wajib Umum"][0]); const [level, setLevel] = useState("SMA"); const [grade, setGrade] = useState("Kelas 10"); const [topic, setTopic] = useState(""); const [subTopic, setSubTopic] = useState(""); const [materialFile, setMaterialFile] = useState<File | null>(null); const [materialText, setMaterialText] = useState(""); const [refImage, setRefImage] = useState<string | null>(null); const [questionCount, setQuestionCount] = useState(10); const [mcOptionCount, setMcOptionCount] = useState<4 | 5>(5); const [difficulty, setDifficulty] = useState<Difficulty>(Difficulty.MEDIUM); const [selectedTypes, setSelectedTypes] = useState<QuestionType[]>([QuestionType.MULTIPLE_CHOICE]); const [selectedCognitive, setSelectedCognitive] = useState<CognitiveLevel[]>([CognitiveLevel.C2, CognitiveLevel.C3, CognitiveLevel.C4]); const [imgQuestionCount, setImgQuestionCount] = useState(0); const [factCheck, setFactCheck] = useState(true); const [readingMode, setReadingMode] = useState<'none' | 'simple' | 'grouped'>('none');
    const [language, setLanguage] = useState<string>('ID');

    useEffect(() => { dbService.getSettings().then(s => setFactCheck(s.ai.factCheck)); }, []);
    useEffect(() => { if (subjectCategory === "Bahasa & Budaya" || subject.includes("Bahasa")) { setReadingMode('grouped'); } else { setReadingMode('none'); } }, [subject, subjectCategory]);
    
    // Auto-select language based on subject, but allow manual override
    const getLangContext = (subj: string) => { if (subj === 'Bahasa Arab') return 'AR'; if (subj === 'Bahasa Jepang') return 'JP'; if (subj === 'Bahasa Korea') return 'KR'; if (subj === 'Bahasa Mandarin') return 'CN'; if (subj === 'Bahasa Jerman') return 'DE'; if (subj === 'Bahasa Perancis') return 'FR'; if (subj === 'Bahasa Inggris') return 'EN'; return 'ID'; };
    useEffect(() => {
        setLanguage(getLangContext(subject));
    }, [subject]);
...
const Login = ({ onLogin }: { onLogin: (u: User) => void }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 p-4 transition-colors">
      <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-xl w-full max-w-md border border-slate-100 dark:border-slate-700 transition-all duration-300">
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-brand-500 rounded-3xl mx-auto flex items-center justify-center text-white font-bold text-3xl mb-4 shadow-xl shadow-brand-500/30">Q</div>
          <h2 className="text-2xl font-extrabold text-slate-800 dark:text-white">Selamat Datang</h2>
          <p className="text-slate-500 text-sm mt-2 font-medium">Masuk untuk mengelola bank soal AI.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <div className="relative">
              <input 
                type="text" 
                required
                className="w-full pl-12 pr-4 py-4 rounded-2xl border border-slate-200 dark:bg-slate-700 dark:border-slate-600 focus:ring-2 focus:ring-brand-500 outline-none transition-all font-medium text-slate-800 dark:text-white placeholder:text-slate-400"
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-brand-600 hover:bg-brand-700 text-white font-bold py-4 rounded-2xl shadow-lg shadow-brand-500/25 transition-all active:scale-95 disabled:opacity-70 flex items-center justify-center gap-2 mt-2 text-lg"
          >
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const checkSession = async () => {
        // Here we rely on simple localStorage for persistence across refreshes for the demo
        // Ideally verify token with dbService if using Turso
  ...
  const handleLogin = (u: User) => {
      setUser(u);
      localStorage.setItem('genz_user', JSON.stringify(u));
  };
  ...
  const updateCredits = (newCredits: number) => {
      if (user) {
          ...
          setUser(updated);
          localStorage.setItem('genz_user', JSON.stringify(updated));
      }
  };
    ...
    <Router>
        ...
        {!user ? (
            ...
            <div className="flex min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors font-sans">
                <Sidebar user={user} isOpen={isSidebarOpen} setIsOpen={setIsSidebarOpen} onLogout={handleLogout} />
                ...
                <div className="flex-1 md:ml-64 flex flex-col min-h-screen transition-all duration-300">
                    <header className="bg-white dark:bg-slate-800 h-16 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between px-4 md:px-8 sticky top-0 z-30 no-print">
                        <div className="flex items-center gap-4">
                             <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-xl md:hidden text-slate-600 dark:text-slate-300">
                                <Menu size={20} />
                            </button>
                            <h1 className="font-bold text-lg text-slate-800 dark:text-white hidden sm:block">
                                {user.role === Role.ADMIN ? 'Administrator Panel' : 'Teacher Dashboard'}
                            </h1>
                        </div>
                        <div className="flex items-center gap-4">
                            ...
                            <div className="w-10 h-10 bg-gradient-to-br from-brand-500 to-brand-600 text-white rounded-xl flex items-center justify-center font-bold shadow-lg shadow-brand-500/20">
                    ...
                    <main className="p-4 md:p-8 flex-1 overflow-x-hidden">
                        <Routes>
                            ...
                            {user.role === Role.ADMIN ? (
                                <>
                                    <Route path="/database" element={<CloudDatabase />} />
                                    <Route path="/users" element={<UserManagement />} />
                                    <Route path="/logs" element={<SystemLogs user={user} />} />
                                    <Route path="/settings" element={<SettingsPage />} />
                                </>
                            ) : (
                                // Protect admin routes
                                <>
                                    <Route path="/database" element={<Navigate to="/dashboard" />} />
                                    <Route path="/users" element={<Navigate to="/dashboard" />} />
                                    <Route path="/logs" element={<Navigate to="/dashboard" />} />
