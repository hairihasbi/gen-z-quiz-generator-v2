        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in-up">
            <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-3xl shadow-2xl p-6 relative overflow-hidden">
                ...
                <div className="text-center">
                    <div className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 ${selectedPackage?.type === 'API_KEY' ? 'bg-indigo-100 text-indigo-600' : 'bg-green-100 text-green-600'}`}>
                        {selectedPackage?.type === 'API_KEY' ? <Key size={32}/> : <Wallet size={32} />}
                    </div>
                    <h3 className="text-2xl font-bold text-slate-800 dark:text-white mb-2">Pembelian {selectedPackage?.type === 'API_KEY' ? 'API Key' : 'Top Up Credit'}</h3>
                    ...
                    <a 
                        href={`https://wa.me/6285248481527?text=Halo Admin, saya ingin membeli ${selectedPackage ? selectedPackage.name : 'Credit'}. Username: ${user.username}. ${selectedPackage ? `Paket: ${selectedPackage.name} (${selectedPackage.credits} ${selectedPackage.type === 'API_KEY' ? 'Keys' : 'Credits'})` : ''}`}
                        target="_blank"
                        rel="noreferrer"
                        className={`w-full text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-lg ${selectedPackage?.type === 'API_KEY' ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/20' : 'bg-green-600 hover:bg-green-700 shadow-green-500/20'}`}
                    >
...
const Dashboard = ({ user }: { user: UserType }) => {
  ...
  const creditPackages: PricingPackage[] = [
      { name: 'Paket Starter', price: 50000, credits: 50, color: 'bg-blue-50 border-blue-100 text-blue-700', type: 'CREDIT' }, 
      { name: 'Paket Pro', price: 100000, credits: 100, color: 'bg-gradient-to-br from-brand-50 to-orange-50 border-brand-200 text-brand-700', type: 'CREDIT' }
  ];

  const keyPackages: PricingPackage[] = [
      { name: 'Paket 5 Key', price: 50000, credits: 5, color: 'bg-indigo-50 border-indigo-100 text-indigo-700', type: 'API_KEY' },
      { name: 'Paket 10 Key', price: 85000, credits: 10, color: 'bg-gradient-to-br from-indigo-50 to-purple-50 border-indigo-200 text-indigo-700', type: 'API_KEY' }
  ];
